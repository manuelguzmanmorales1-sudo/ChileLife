const AupolUI = {
  ...
  renderResultadoConsulta() {
    ...
    return `
      ...
      <div class="card">
        ...
        <button class="btn btn-sm btn-outline" style="margin-top:10px;" onclick="AupolUI.abrirFormDetencion('${data.ciudadano.rut}','${data.ciudadano.nombre.replace(/'/g,"")}')"><i class="fas fa-handcuffs"></i> Registrar Detención</button>
      </div>
    ...
    try {
      ...
      if (e.message && e.message.includes('orden de allanamiento')) {
        cont.innerHTML = `
          ...
          <button class="btn btn-danger" onclick="AupolUI.solicitarOrden('${rut}')"><i class="fas fa-gavel"></i> Solicitar Orden y Ver Ficha</button>
        `;
      } else {
  ...
  abrirFormDetencion(rut, nombre) {
    App.showModal('Registrar Detención', `
      <form onsubmit="event.preventDefault();AupolUI.guardarDetencion('${rut}')">
  ...
  async guardarDetencion(rut) {
    ...
    try {
      await API.crearDetencion({ rutDetenido: rut, motivo, institucion: 'Carabineros' });
